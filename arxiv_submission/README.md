...
This folder is a self-contained de-anonymized arXiv preprint package.
...
* **Format**: ACL-style preprint (`\usepackage{acl}`), without review rulers or line numbers.
* **Authorship**: de-anonymized author block, plus code/data and project-page links in the title note.
...
* **Sections**: `Limitations` and `Ethical Considerations` are present.
...
By default, the script looks for CSVs in either
`../semantic_stroop_final_reproduction_data_package/csv/` or `../data/csv/`.
To use another data-package location, set `SEMANTIC_STROOP_CSV_DIR` to the
desired `csv/` directory before running the script.
...
| 6      | column       | `mechanism_random_controls.csv`                                             |
...
project data package (`../data/csv/` in the GitHub repository, or
`../semantic_stroop_final_reproduction_data_package/csv/` in the standalone workspace).
